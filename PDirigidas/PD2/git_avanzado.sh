...
# 12) Generar un reporte de estado del repositorio.
# 13) Salir.
#
    ...
    while true; do
        ...
        case "$opcion_rama" in
            ...
            e|E)
                ...
                read rama_actual
                echo -n "Ingrese el nuevo nombre de la rama: "
                read nuevo_nombre
    ...
    while true; do
        ...
        case "$opcion_diff" in
            ...
            d|D)
                ...
                echo -n "Ingrese el segundo identificador (rama o commit): "
                read id2
                echo -n "Ingrese la ruta del archivo a comparar: "
                read archivo
                echo ""
...
# 11. Merge automatizado
function merge_automatizado() {
    echo ""
    echo "=== Merge automatizado con resolución de conflictos ==="
    echo ""
    echo -n "Ingrese el nombre de la rama a fusionar en la actual: "
    read rama_origen

    echo -n "¿Qué estrategia de resolución desea usar? [ours/theirs(por defecto)] : "
    read estrategia
    estrategia=${estrategia:-theirs}

    git merge -X "$estrategia" "$rama_origen"

    if [[ $? -eq 0 ]]; then
        echo "Merge completado automáticamente utilizando la estrategia '$estrategia'."
    else
        echo "Hubo conflictos que no pudieron resolverse automáticamente."
    fi
}
# 12. Generar reporte de estado del repositorio
function generar_reporte() {
    archivo="reporte_git.txt"
    echo "Generando reporte en '$archivo'..."
